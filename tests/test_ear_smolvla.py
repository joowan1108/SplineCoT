...
def test_broad_ear_resamples_but_short_action_keeps_real_ticks():
    ...
    state[..., 3] = state[..., 10] = 1
    current, ear, action, mask = build_pose_targets(
        state, ear_samples=64, action_samples=16
    )
...
def test_quadratic_spline_is_c1_and_handoff_is_hard_anchored():
    ...
    start, velocity, duration = torch.randn(2, 4), torch.randn(2, 4), 0.2
    anchored = spline.constrain_start(params, start, velocity, duration)
    torch.testing.assert_close(anchored[:, 0], start)
    first_derivative, _ = spline.endpoint_derivatives(anchored)
    torch.testing.assert_close(first_derivative / duration, velocity)
...
def test_overlap_selection_exposes_each_segments_three_real_controls():
    ...
    guidance = spline.select_parameter_guidance(
        params, torch.tensor([0.26]), phase_span=0.24
    )
    ...
    assert guidance.valid.all() and guidance.available.all()
    assert guidance.role.tolist() == [[0, 1, 2]]


def test_teacher_mask_modes_are_no_mask_partial_and_learned_null_inputs():
    available = torch.ones(3, 6, dtype=torch.bool)
    masked = random_teacher_guidance_mask(
        available, (1 / 3, 1 / 3, 1 / 3), modes=torch.tensor([0, 1, 2])
    )
    assert masked[0].all() and masked[1].any() and not masked[1].all() and not masked[2].any()
...
def test_mc_parameter_covariance_propagates_through_spline_basis():
    ...
    torch.testing.assert_close(
        trajectory_variance, decoded.var(dim=0, unbiased=True), atol=2e-5, rtol=2e-5
    )
...
def test_action_parameter_attention_blocks_upstream_gradients():
    ...
    ear = QuadraticSpline(2, 10).select_parameter_guidance(
        torch.randn(2, 4, 3, requires_grad=True), torch.zeros(2), 0.5
    ).detached()
    output = expert(
        torch.randn(2, 4, 3), torch.rand(2), context, ear, torch.zeros(2), 0.5
    ).sum()
    output.backward()
...
def test_fast_tokenizer_round_trip_contract():
    ...
    processor = BatchProcessor(
        small_config(), tokenizer=TextTokenizer(), action_tokenizer=ActionTokenizer()
    )
...
def test_libero_processor_builds_fixed_future_pose_splines():
    ...
    processor = LIBEROBatchProcessor(
        config, tokenizer=TextTokenizer(), action_tokenizer=ActionTokenizer()
    )
